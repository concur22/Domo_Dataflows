-- available tables: 
-- applications, offers, job_stages, job_openings, job_openings_w_entity, jobs, job_recruiters

-- First table created: applications_join_1
SELECT		
...
FROM `Applications` a
LEFT JOIN `Job Stages` js on a.current_stage_id = js.id;

-- Second table created: job_openings_join_1
SELECT 
  jo.id AS "Job ID"
...
-- Third table created: applications_join_2
SELECT DISTINCT
	 a."Application ID"
   , a."jobs_id"
   , a."Source ID"
...
WHERE "Is Job Openings Template" NOT ILIKE '%true%';

-- Fourth table created: applications_join_3
SELECT a.* 
    , j.id as "Jobs ID"
...
-- Fifth table created: offers_select1
SELECT *
FROM (
  SELECT
    o.id AS "Offer ID",
...
-- Last table created: applications_final_select
  SELECT 
    "Jobs ID"
    , "Job Openings ID"
    , "Application ID"
...
SELECT * FROM "applications_final_select"
